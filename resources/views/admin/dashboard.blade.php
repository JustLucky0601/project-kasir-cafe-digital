...
@push('scripts')
<script>
const chartDailyLabels = {!! json_encode($chartDailyLabels) !!};
const chartDailyData = {!! json_encode($chartDailyData) !!};
const chartWeeklyLabels = {!! json_encode($chartWeeklyLabels) !!};
const chartWeeklyData = {!! json_encode($chartWeeklyData) !!};
const chartMonthDailyLabels = {!! json_encode($chartMonthDailyLabels) !!};
const chartMonthDailyData = {!! json_encode($chartMonthDailyData) !!};

// Bulanan (total pendapatan per bulan pada tahun berjalan)
const chartMonthTotalLabels = {!! json_encode(range(1,12)) !!};
const chartMonthTotalLabelsFormatted = chartMonthTotalLabels.map(m => formatMonthLabel(m));
const chartMonthTotalValues = {!! json_encode($chartMonthTotalValues ?? array_fill(0,12,0)) !!};
